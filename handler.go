...
type Logger func(method string, params []reflect.Value, results []reflect.Value, err error)
...
func (s *handler) handle(ctx context.Context, req request, w func(func(io.Writer)), rpcError rpcErrFunc, done func(keepCtx bool), chOut chanOut) {
	...
	if err != nil {
		...
		stats.Record(ctx, metrics.RPCRequestError.M(1))
		if s.Logger != nil {
			s.Logger(req.Method, callParams, nil, err)
		}
		return
	}
	if req.ID == nil {
	...
	if s.Logger != nil {
		s.Logger(req.Method, callParams, callResult, nil)
	}
	// /////////////////
